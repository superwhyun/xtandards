...
interface MinutesSelectDialogProps {
  isOpen: boolean
...
export default function MinutesSelectDialog({ isOpen, onOpenChange, onGenerateMinutes }: MinutesSelectDialogProps) {
  const { t } = useLanguage()
  ...
  const handleGenerate = () => {
    if (selectedTitle) {
      onGenerateMinutes(selectedTitle)
      setSelectedTitle("")
      onOpenChange(false)
    }
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>회의록 생성할 회의명 선택</DialogTitle>
        </DialogHeader>
